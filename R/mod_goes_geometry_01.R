...
# UI
# ============================================================
mod_goes_geometry_01_ui <- function(id) {
  ns <- shiny::NS(id)
  ...
  shiny::tagList(
    shiny::tags$head(
      shiny::tags$script(src = "https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"),
      shiny::tags$script(src = "https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.js"),
      shiny::tags$style(shiny::HTML(earth3d_css()))
    ),
    ...
    shiny::div(
      ...
      shiny::div(
        ...
        shiny::actionButton(ns("toggle_gis"), "GIS / Cartografía", class = "menu-toggle-btn collapsed"),
        shiny::div(
          id = ns("gis_options"),
          style = "display: none;",
          shiny::h4("Referencia visual"),
          shiny::selectInput(
            ns("reference_axis"),
            "Eje de referencia",
            choices = c(
          ...
          shiny::tags$div(class = "geo-sat-note", "Si elegís eje terrestre, toda la escena rota para dejar el eje norte-sur vertical."),
          shiny::tags$hr(),
          shiny::h4("Textura / mapa"),
          shiny::selectInput(ns("earth_texture_file"), "Textura base WGS84 solid", choices = character(0)),
          shiny::selectInput(ns("earth_overlay_file"), "Capa WGS84 transparente", choices = c("Ninguna" = "")),
          shiny::checkboxInput(ns("show_texture"), "Mostrar textura base", value = TRUE),
          shiny::checkboxInput(ns("show_overlay"), "Mostrar capa transparente", value = TRUE),
          shiny::checkboxInput(ns("show_blue_sphere"), "Mostrar esfera azul base", value = TRUE),
          shiny::checkboxInput(ns("show_graticule"), "Mostrar malla meridianos/paralelos", value = FALSE),
          shiny::checkboxInput(ns("show_stars"), "Mostrar estrellas", value = TRUE),
          shiny::checkboxInput(ns("show_sun_arrow"), "Mostrar dirección solar", value = TRUE),
          shiny::checkboxInput(ns("show_subsolar_point"), "Mostrar punto subsolar", value = TRUE),
          shiny::checkboxInput(ns("show_earth_axis"), "Mostrar eje terrestre", value = TRUE),
          shiny::checkboxInput(ns("show_polar_terminator_axis"), "Eje al plano de traslación", value = TRUE),
          shiny::checkboxInput(ns("show_equator"), "Mostrar ecuador destacado", value = TRUE),
          shiny::checkboxInput(ns("show_greenwich"), "Mostrar meridiano de Greenwich", value = TRUE),
          shiny::tags$hr(),
          shiny::h4("Iluminación"),
          shiny::sliderInput(ns("night_light"), "Iluminación del lado oscuro", min = 0, max = 100, value = 12, step = 1),
          shiny::sliderInput(ns("day_light"), "Intensidad del lado iluminado", min = 0, max = 200, value = 108, step = 1),
          shiny::sliderInput(ns("day_gradient"), "Gradiente luz día centro-borde", min = 0, max = 100, value = 35, step = 1),
          shiny::tags$hr(),
          shiny::h4("Capas geográficas"),
          shiny::checkboxInput(ns("show_utc0"), "Mostrar zona UTC±0", value = FALSE),
          shiny::checkboxInput(ns("show_subsolar_timezone"), "Resaltar huso UTC del punto subsolar", value = FALSE),
          shiny::tags$hr(),
          shiny::h4("Elementos físicos"),
          shiny::checkboxInput(ns("show_terminator"), "Mostrar terminador", value = FALSE),
          shiny::checkboxInput(ns("show_sun_center_line"), "Mostrar haz solar al centro", value = TRUE),
          shiny::checkboxInput(ns("show_earth_center"), "Mostrar centro de la Tierra", value = TRUE),
          shiny::checkboxInput(ns("show_equator_plane"), "Mostrar plano ecuatorial interno", value = TRUE),
          shiny::checkboxInput(ns("show_greenwich_plane"), "Mostrar plano interno de Greenwich", value = TRUE),
          shiny::checkboxInput(ns("show_greenwich_equator_vector"), "Mostrar vector centro → Greenwich/Ecuador", value = TRUE),
          shiny::checkboxInput(ns("show_solar_equator_projection"), "Mostrar proyección solar sobre el ecuador", value = TRUE),
          shiny::checkboxInput(ns("show_solar_meridian"), "Mostrar meridiano de mediodía solar", value = FALSE),
          shiny::tags$hr()
        )
      ),
      ...
      shiny::div(
        ...
        shiny::actionButton(ns("toggle_sat"), "Satélites GEO", class = "menu-toggle-btn collapsed"),
        shiny::div(
          id = ns("sat_options"),
          style = "display: none;",
          shiny::h4("Geoestacionarios locales"),
          shiny::checkboxInput(ns("show_geo_sats"), "Mostrar sistema GEO", value = TRUE),
          shiny::checkboxInput(ns("show_geo_orbits"), "Mostrar órbitas", value = TRUE),
          shiny::checkboxInput(ns("show_geo_footprints"), "Mostrar coberturas", value = TRUE),
          shiny::checkboxInput(ns("show_geo_cones"), "Mostrar conos", value = TRUE),
          shiny::checkboxInput(ns("show_geo_vectors"), "Mostrar líneas Tierra → satélite", value = TRUE),
          shiny::checkboxInput(ns("show_geo_labels"), "Mostrar nombres", value = TRUE),
          shiny::tags$hr(),
          shiny::h4("GOES-18"),
          shiny::checkboxInput(ns("show_geo_goes18_enabled"), "Activar GOES-18", value = FALSE),
          shiny::checkboxInput(ns("show_geo_goes18_orbit"), "Órbita GOES-18", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes18_footprint"), "Cobertura GOES-18", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes18_cone"), "Cono GOES-18", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes18_vector"), "Línea GOES-18", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes18_label"), "Nombre GOES-18", value = TRUE),
          shiny::h4("GOES-19"),
          shiny::checkboxInput(ns("show_geo_goes19_enabled"), "Activar GOES-19", value = FALSE),
          shiny::checkboxInput(ns("show_geo_goes19_orbit"), "Órbita GOES-19", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes19_footprint"), "Cobertura GOES-19", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes19_cone"), "Cono GOES-19", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes19_vector"), "Línea GOES-19", value = TRUE),
          shiny::checkboxInput(ns("show_geo_goes19_label"), "Nombre GOES-19", value = TRUE),
          shiny::tags$hr(),
          shiny::tags$div(
            class = "geo-sat-note",
            "Estos satélites no usan internet. Se dibujan con longitud fija sobre el ecuador. La distancia orbital es proporcional al radio terrestre: 42164 / 6371 ≈ 6.62 radios terrestres."
          ),
          shiny::tags$div(
            class = "geo-sat-note",
            shiny::tags$b("Incluidos:"),
            shiny::tags$br(),
            "GOES-19 / GOES-East: 75.2°O",
            shiny::tags$br(),
            "GOES-18 / GOES-West: 137.0°O"
          )
        )
      ),
      ...
      shiny::div(
        ...
        shiny::actionButton(ns("toggle_sim"), "Simulación / Procesamiento", class = "menu-toggle-btn collapsed"),
        shiny::div(
          id = ns("sim_options"),
          style = "display: none;",
          shiny::h4("Modo de trabajo"),
          shiny::radioButtons(
            ns("position_mode"),
            "¿Qué querés hacer?",
            choices = c(
          ...
          shiny::conditionalPanel(
            ...
            shiny::tags$div(class = "mode-note", "Modo seguimiento: la posición subsolar y la orientación de la Tierra se recalculan usando la fuente de tiempo seleccionada.")
          ),
          shiny::conditionalPanel(
            condition = sprintf("input['%s'] == 'simulation'", ns("position_mode")),
            shiny::tags$div(class = "mode-note", "Modo simulación: no se consulta la hora del sistema para actualizar posiciones; se usa un reloj interno simulado controlado con Play, Pausa y Stop.")
          ),
          shiny::tags$hr(),
          shiny::h4("Fuente de datos / tiempo"),
          shiny::radioButtons(
            ns("time_source"),
          ...
          shiny::tags$div(class = "mode-note", "En seguimiento, esta fuente se usa en cada recálculo. En simulación, esta fuente define la hora inicial desde la que empieza a avanzar el reloj simulado."),
          shiny::conditionalPanel(
            condition = sprintf("input['%s'] == 'manual'", ns("time_source")),
            shiny::tags$div(
              class = "manual-time-row",
              shiny::numericInput(ns("manual_year"), "Año", value = lubridate::year(now_utc), min = 1900, max = 2100, step = 1),
              shiny::numericInput(ns("manual_month"), "Mes", value = lubridate::month(now_utc), min = 1, max = 12, step = 1),
              shiny::numericInput(ns("manual_day"), "Día", value = lubridate::day(now_utc), min = 1, max = 31, step = 1),
              shiny::numericInput(ns("manual_hour"), "Hora", value = lubridate::hour(now_utc), min = 0, max = 23, step = 1),
              shiny::numericInput(ns("manual_minute"), "Minuto", value = lubridate::minute(now_utc), min = 0, max = 59, step = 1),
              shiny::numericInput(ns("manual_second"), "Segundo", value = lubridate::second(now_utc), min = 0, max = 59, step = 1)
            )
          ),
          shiny::p("Fuente usada: ", shiny::span(id = ns("time_source_txt"), class = "value")),
          shiny::p("UTC usado: ", shiny::span(id = ns("utc_txt"), class = "value")),
          shiny::conditionalPanel(
            condition = sprintf("input['%s'] == 'tracking'", ns("position_mode")),
            shiny::radioButtons(
              ns("refresh_seconds"),
            ...
            shiny::actionButton(ns("refresh_now"), "Actualizar ahora")
          ),
          shiny::conditionalPanel(
            condition = sprintf("input['%s'] == 'simulation'", ns("position_mode")),
            shiny::tags$div(
              style = "display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px; margin-bottom: 8px;",
              shiny::actionButton(ns("sim_play"), "▶ Play", style = "background:#22c55e;color:white;font-weight:bold;border:0;border-radius:8px;"),
              shiny::actionButton(ns("sim_pause"), "⏸ Pausa", style = "background:#f59e0b;color:#111;font-weight:bold;border:0;border-radius:8px;"),
              shiny::actionButton(ns("sim_stop"), "⏹ Stop", style = "background:#ef4444;color:white;font-weight:bold;border:0;border-radius:8px;")
            ),
            shiny::radioButtons(
              ns("simulation_speed"),
          ...
          shiny::tags$hr(),
          shiny::h4("Sol y punto subsolar"),
          shiny::p("Latitud subsolar: ", shiny::span(id = ns("lat_txt"), class = "value")),
          shiny::p("Longitud subsolar: ", shiny::span(id = ns("lon_txt"), class = "value")),
          shiny::p("Hora solar allí: ", shiny::span(id = ns("solar_time_txt"), class = "value")),
          shiny::p("Huso UTC ideal: ", shiny::span(id = ns("subsolar_utc_zone_txt"), class = "value")),
          shiny::p("Hora civil aproximada: ", shiny::span(id = ns("subsolar_clock_txt"), class = "value")),
          shiny::p("Inclinación terrestre: ", shiny::span("23.44°", class = "value")),
          shiny::tags$hr(),
          shiny::h4("Referencias físicas"),
          shiny::p(shiny::span("Rojo:", class = "red"), " eje de rotación norte-sur de la Tierra."),
          shiny::p(shiny::span("Blanco:", class = "white"), " terminador día/noche y centro terrestre."),
          shiny::p(shiny::span("Verde:", class = "green"), " plano ecuatorial interno."),
          shiny::p(shiny::span("Azul:", class = "blue"), " plano interno de Greenwich."),
          shiny::p(shiny::span("Naranja:", class = "orange"), " proyección solar sobre el ecuador."),
          shiny::p(shiny::span("Amarillo:", class = "value"), " Sol, punto subsolar, haz solar y mediodía solar.")
        )
      ),

      shiny::div(
        class = "earth-floaters-panel",
        shiny::actionButton(ns("toggle_floaters"), "Ocultar objetos móviles", class = "small-toggle-btn")
      ),

      shiny::div(
        id = ns("utc_clock_badge"),
        class = "floating-time-badge utc-clock-badge",
        shiny::span(class = "badge-label", "UTC"),
        shiny::span(class = "badge-value", "--:--:--")
      ),

      shiny::div(
        id = ns("last_position_update_badge"),
        class = "floating-time-badge last-update-badge",
        shiny::span(class = "badge-label", "Última actualización posición"),
        shiny::span(class = "badge-value", "--")
      ),

      shiny::div(
        id = ns("next_position_update_badge"),
        class = "floating-time-badge next-update-badge",
        shiny::span(class = "badge-label", "Próxima actualización en"),
        shiny::span(class = "badge-value", "--")
      ),

      shiny::div(
        id = ns("orbit_window"),
        class = "floating-visual-window orbit-window",
        shiny::div(class = "window-title", "Órbita anual Tierra-Sol"),
        shiny::div(
          class = "window-body",
          shiny::tags$canvas(id = ns("orbit_canvas"), width = 760, height = 430)
        )
      ),

      shiny::div(
        id = ns("map_window"),
        class = "floating-visual-window map-window",
        shiny::div(class = "window-title", "Mapa WGS84 día / noche"),
        shiny::div(
          class = "window-body",
          shiny::tags$canvas(id = ns("map_canvas"), width = 920, height = 460)
        )
      ),

      shiny::div(id = ns("earth3d"), class = "earth3d-container")
    ),

    shiny::tags$script(shiny::HTML(earth3d_js(ids_json, ns)))
  )
}
...
# Server
# ============================================================
mod_goes_geometry_01_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::addResourcePath(resource_prefix, bg_layers_dir)
    ...
    get_position_mode <- function() {
      ...
      if (is.null(mode) || length(mode) == 0 || !(mode %in% c("tracking", "simulation"))) {
        return("tracking")
      }
      mode
    }
    ...
    get_refresh_seconds <- function() {
      ...
      if (is.null(x) || length(x) == 0 || is.na(x) || !(x %in% c(1, 5, 10, 30))) {
        return(5)
      }
      x
    }
    ...
    send_sun_update <- function() {
      position_mode <- shiny::isolate(get_position_mode())
      time_source_code <- shiny::isolate(input$time_source)
      if (is.null(time_source_code) || length(time_source_code) == 0) {
        time_source_code <- "system"
      }

      time_info <- shiny::isolate(selected_utc_time())
      refresh_seconds <- shiny::isolate(get_refresh_seconds())
    ...
    shiny::observe({
      ...
      if (is.null(time_source) || length(time_source) == 0) {
        time_source <- "system"
      }
    ...
    send_simulation_control <- function(action) {
      ...
      if (is.null(speed) || length(speed) == 0 || is.na(speed) || speed <= 0) {
        speed <- 600
      }

      session$sendCustomMessage(
        session$ns("autorotate_update"),
        list(
          action = action,
    ...
    shiny::observeEvent(input$refresh_now, {
      shiny::updateRadioButtons(session, "position_mode", selected = "tracking")
      shiny::updateRadioButtons(session, "time_source", selected = "system")
      send_simulation_control("stop")
      send_sun_update()
    }, ignoreInit = TRUE)

    shiny::observeEvent(input$sim_play, {
      shiny::updateRadioButtons(session, "position_mode", selected = "simulation")
      send_simulation_control("play")
    }, ignoreInit = TRUE)

    shiny::observeEvent(input$sim_pause, {
      shiny::updateRadioButtons(session, "position_mode", selected = "simulation")
      send_simulation_control("pause")
    }, ignoreInit = TRUE)

    shiny::observeEvent(input$sim_stop, {
      shiny::updateRadioButtons(session, "position_mode", selected = "tracking")
      shiny::updateRadioButtons(session, "time_source", selected = "system")
      send_simulation_control("stop")
      send_sun_update()
    ...
    shiny::observeEvent(input$simulation_speed, {
      send_simulation_control("speed")
    }, ignoreInit = TRUE)

    shiny::observeEvent(input$toggle_floaters, {
      session$sendCustomMessage(session$ns("toggle_floaters_update"), list())
    }, ignoreInit = TRUE)

    shiny::observe({
      session$sendCustomMessage(
        session$ns("night_light_update"),
        list(value = input$night_light)
      )
    })

    shiny::observe({
      mode <- input$reference_axis
      if (is.null(mode) || length(mode) == 0 || !(mode %in% c("solar_axis", "earth_axis"))) {
        mode <- "solar_axis"
      }

      session$sendCustomMessage(
        session$ns("reference_axis_update"),
        list(mode = mode)
      )
    })
    ...
    shiny::observe({
      session$sendCustomMessage(
        ...
        list(
          dayLight = input$day_light,
          dayGradient = input$day_gradient
        )
      )
    })
    ...
    shiny::observeEvent(input$earth_texture_file, {
      shiny::req(input$earth_texture_file)

      texture_url <- file.path(
        resource_prefix,
        "bg_01_wgs84_01_solid",
        input$earth_texture_file
      )

      texture_url <- gsub("\\\\", "/", texture_url)

      session$sendCustomMessage(
        session$ns("earth_texture_update"),
        list(url = texture_url)
      )
    }, ignoreInit = TRUE)

    shiny::observeEvent(input$earth_overlay_file, {
      ...
      session$sendCustomMessage(
        session$ns("earth_overlay_update"),
        list(url = overlay_url)
      )
    }, ignoreInit = TRUE)
  })
}
